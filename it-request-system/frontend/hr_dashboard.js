...
function displayRequests(requests, filter = "all") {
  ...
  filteredRequests.forEach((request) => {
    ...
    // New condition: disable HR actions if item is Printer and ED approval is not done
    const isEDApproved = request.status.ed === "approved";
    const disableHRActions = isHRPending && (!isHODApproved || (request.item === "Printer" && !isEDApproved));
                ...
                ${
                  request.status.hr === "pending" && !request.actionedViaEmail?.hr
                    ...
                    <button class="btn btn-sm btn-outline-success action-btn approve" 
                        ...
                        ${disableHRActions ? ' (Awaiting HOD/PH)' : ''}
                    </button>
                    ...
                    ${
                      disableHRActions 
                        ? '<div class="small text-muted mt-1">Requires HOD and Plant_Head approval first</div>' 
                        : ''
                    }
                `
